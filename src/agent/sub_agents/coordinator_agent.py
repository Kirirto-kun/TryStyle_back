...
from .base import get_azure_llm, AgentResponse, ProductList, Outfit, GeneralResponse
...
async def search_products(ctx: RunContext[int], user_message: str) -> ProductList:
    """
...
@coordinator_agent.tool  
async def recommend_outfit(ctx: RunContext[int], user_message: str) -> Outfit:
    """
...
@coordinator_agent.tool
async def handle_general_query(ctx: RunContext[int], user_message: str) -> GeneralResponse:
    """
    ...
    """
    result = await general_agent.run(user_message)
    return result.data


async def coordinate_request(message: str, user_id: int) -> AgentResponse:
    """
    ...
    try:
        ...
        result = await coordinator_agent.run(message, deps=user_id)
        return result.data
